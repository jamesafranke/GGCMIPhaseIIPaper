...
\begin{figure}[h!]
  ...
  \caption{
  As in Figure S3 except now for daily maximum temperature. Changes in daily maximum temperature variability are generally higher than those for daily minimum temperature.
  }
  \label{fig:var}
\end{figure}
...
\begin{justify}
In this section we present maps of cross validation error (values found in main text Table 3 are aggregated up from the grid cell level). Errors are generally low as a percentage of yield change in each grid cell. Errors can be above 10\% of yield change for the out-of-sample test in some regions for some crops, most notably spring wheat in southern China in the PROMET model.
\end{justify}
