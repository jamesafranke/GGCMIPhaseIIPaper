...
\begin{figure}[h!]
  ...
  \caption{
  Change in hemispheric-summer variability (JJA in the north or DJF in the south) for daily mean temperature from six CMIP-5 models for an abrupt 4x CO$_2$ forcing. Heatmap shows the ratio in standard deviation in future projected daily temperature compared to preindustrial daily temperature values. 
  The HadCM3 model shows relatively strong variability changes, though climate model disagreement about changes in variability is substantial.
  }
  \label{fig:lpjmlrcp}
\end{figure}
...
\begin{figure}[h!]
  ...
  \caption{
  Illustration of the factors affecting yields in more realistic climate scenarios for spring wheat. Same convention main text Figure 11. Emulator errors in PROMET are linked to Southern China, where error are very high due to discontinuities in yield response in that region. (See figure in supplement section 10)
  }
\end{figure}
...
\begin{figure}[h!]
  ...
  The split in PROMET response panel \textbf{a} illustrates the differences in spatial patterns of temperature change across climate models. 
  }
\end{figure}
...
In this section we present maps of cross validation error (values found in main text Table 3). Errors are generally low as a percentage of yield change in each grid cell.
