...
\begin{document}
\title{Globally-gridded mean climatological crop model emulators for yield (and irrigation water demand??)}

\Author[1,2]{James}{Franke}
\Author[3]{Christoph}{M\"{u}ller}
\Author[2,4]{Joshua}{Elliott}
\Author[5]{Alexander C.}{Ruane}
...
\introduction
\label{S:1}
Efforts to project climate change impacts often combine models from different disciplines under a single economic framework to calculate a total cost of climate change often called an Integrated Assessment Model (IAM). 
In general, the numerical models implemented by different disciplines studying climate change and its associated impacts often cannot be directly integrated into IAMs because of their simulation framework or computational cost.
...
Statistical crop models on the other hand, lack some of the important features of process-based models including future atmospheric CO$_2$ fertilization and potential changes in agricultural management, but sometimes take analytical forms that can be brought directly into an IAM. 
