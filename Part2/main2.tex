...
\begin{document}
\title{Globally-gridded mean climatological crop model emulators for yield (and irragation water demand)} %and irrigation water demand
...
\introduction
\label{S:1}
Efforts to project climate change impacts often combine models from several different disciplines under a unified economic framework to provide a total cost of climate change or total cost of carbon. Where the so-called Integrated Assessment Models (IAMs) excel in breadth they lack in depth. The numerical models used by the different disciplines studying climate change and its associated impacts cannot be directly integrated into IAMs because of their simulation framework or computational cost. Process-based crop simulation models provide many advantages for projecting the impacts of climate change on agriculture but fall into the category of models difficult to integrate into larger economic assessments. Statistical crop models on the other hand, often lack important features of process-based models including future CO$_2$ direct fertilization and potential changes in management, but sometimes take analytical forms that can be brought directly into an IAM.
...
Other statistical models tested include a Gaussian process model. A Gaussian process model can be fit to the response in a single grid cell, and generally outperforms the polynomial in terms of out of sample prediction, but quickly becomes cumbersome in the context of fitting individual kernels across 40,000 grid cells for a single simulation model and crop.
