...
%The following point was mentioned in the response letter, so better keep it:
...
Our emulators are trained only on growing-season means, and the GGCMI Phase~2 exercise involved only changes in means. We therefore seek to assess whether changes in the higher moments of temperature and precipitation distributions in a climate projection might have effects that lead to significant emulator error. Note that we are not asking whether year-over-year climate variability matters to crop yields; this point is well-established \citep{ray2015climate}. The question instead is whether a realistic future climate projections involves \textit{changes} in variablity large enough that they compromise an emulator based on the GGCMI Phase~2 dataset. 
...
\label{S:6}
In this work we describe a new class of global gridded crop model emulators for 5 crops (maize, soybean, rice and spring and winter wheat) and 9 process-based crop models, based on the GGCMI Phase~2 dataset.
