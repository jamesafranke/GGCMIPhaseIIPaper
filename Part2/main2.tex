...
\begin{document}
\title{GGCMI Phase II: global gridded crop model emulators}
...
In the following sections we descibe the training dataset, the model stastical specificaction, and some evidence of model fidelity.

%The spatially explicit emulator at the mean-climatological time scale that allows for representing process-based crop model responses in economic assessment models and other applications.
%%% SOMETHING HERE %%%%%%%%%
...
Atmospheric CO$_2$ and applied nitrogen fertilizer varied as well and take uniform values globally for each simulation case. 
...
%% this is weird, cut and past from elsewhere??
...
We do not focus on comparing different functional forms in this study, and instead choose a relatively simple paramteric model with a polynomial basis function that allows for some interpretation of parameter weights. 
...
The emulation therefore indirectly includes any yield response to geographically distributed factors such as soil type, insolation, and the baseline climate, and preserves the spatail resolution of the parent models.
We hold the statistical specification constant across all crops and models to facilitate potential parameter-by-parameter simulation model comparison.
...
Feature importance is not unifom due to spatial heterogenity across models and crops, so we weight the loss function by current cultuivation area durring this step. 
...
To fit the parameters $K$, we use a Bayesian Ridge regulaization method \citep{MacKay91}, which reduces volatility in parameter estimates when the sampling is sparse, by weighting parameter estimates towards zero. 
...
Model emulation with the paramteric method proposed here is only possible when crop yield responses are sufficiently smooth and continuous to allow fitting with a relatively simple functional form, but this condition largely holds in the GGCMI Phase II simulations. 
