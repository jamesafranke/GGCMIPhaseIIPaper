...
Crops are less sensitive to short-term precipitation changes, since they care about soil moisture and the soil integrates. Previous work has shown that in all but very arid regions, crop responses in models are not sensitive to how the precipitation is distributed within a month \citep{Glotter14}.
...
We have nearly 3 million different regressions to summarize because we fit individual models at each grid cell for each simulation model and crop.
...
\centering
    ...
    \caption{I
    llustration of our test of emulator performance, applied to the CARAIB model for the T+4 scenario for rainfed crops. 
...
\textbf{JULES}     & 3.99  & 20.1  & 2.89  & 9.90  & NA     \\ \hline
...
\textbf{PROMET}    & 4.41  & 7.48  & 6.06  & 16.8  & 7.07   \\ \hline
\textbf{PEPIC}     & 1.30* & 0.72* & 1.16* & 0.63* & 1.65*  \\ \hline
\end{tabular}
\end{table*}

We also provide a more stringent test of emulator performance; a four-fold cross validation (out-of-sample validation). 
Here the training data is split and the model is trained on 75\% of the data and tested on the held out portion (the process is then repeated four times to cover all data in the training set). 
We normalize the error in each grid cell by dividing by the yield in that grid cell in the baseline (T+0, W+0, C=360, N=200) simulation and show aggregations weighted by area cultivated per grid cell. 
Errors are generally low as a percentage of yield --even for this relatively strict protocol-- and when weighted by area, below 5\% of baseline yield for most cases (Table \ref{table:ASE}).
The notable exceptions are the JULES model for soy and spring wheat and the PROMET model for soy, rice, and the wheats. 
...
\begin{figure*}[ht]
    \centering
    \includegraphics[width = 14cm]{figures/global_em_maize_4.png}
    ...
    Panels a-d show CMIP-5 temperature only, temperature and precipitation, temperature and CO$_2$, and all three effects respectively. 
    Rug plots show end of century (2090-2100 mean) values for temperature change for each CMIP-5 model and resultant production change for each crop and climate model combination (including the additional six crop model emulators not shown in full).
...
The emulator or ``surrogate model'' transforms the discrete simulation samples into a continuous response surface at any geographic scale and can be used for a variety of applications, including construction of continuous damage functions. 
...
The emulated crop model yield responses to business-as-usual climate change (Representative Concentration Pathway (RCP) 8.5) are shown in Figure \ref{fig:globe_em} for 30 climate models from the CMIP-5 archive \citep{Taylor2012}. 
Differences between the emulation and simulation are similar to the spread across climate models for a single crop model emulation, but much lower than the differences across climate models. 

With CO$_2$ and precipitation effects included, the climate model response spread increases considerably. 
Note that no direct comparison to the simulations is possible in cases b, c, or d.

Precipitation tends 

PROMET, the quantitatively most difficult model to emulate for Maize is shown to illustrate that ...

PROMET, the pattern of yields 
