...
The emulators presented here capture the responses of process-based crop models in a lightweight form. While an emulator will never be more \textit{accurate} then its parent model, it can have multiple advantages over a computationally intensive numerical simulation. 
...
Third, neither growing season specification tested in GGCMI Phase 2 (A0 and A1) accounts for a major potential adaptation pathway under climate change, a shift to earlier or later planting dates \citep{waha2012,minoli2019modeling}. 
...
And finally, the emulator should not be used to predict individual yearly yields, as the forced climatological mean yield response does not necessarily track with the yield response to mean growing season weather in a single year. The emulator cannot then provide a measure of changing yield variance, and should not be used to evaluate extremes.
