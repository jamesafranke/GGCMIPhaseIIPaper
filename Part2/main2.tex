...
See Figure S3 in the supplement for number of simulations associated with each combination of input levels.
Table repeated from Franke et al.
}
\label{table:inputs} 
    \begin{tabular}{lcc} 
        \tophline \vspace{1mm}
...
The GGCMI Phase II simulations that serve as the training dataset for the crop model emulators presented here are described in detail in Franke et al \textcolor{red}{(this issue)}, but we summarize briefly here.
The emulator training dataset consists of the outputs from nine process-based crop models for five different crops (maize, rice, soybean, spring wheat, and winter wheat), each simulated globally at the 0.5 degree latitude and longitude resolution.
...
The models included in the GGCMI emulators can be found in Table \ref{table:models}. 

Most process-based models in GGCMI Phase II utilize the AgMERRA \citep{Ruane2015} historical daily climate data product for input weather.
The PROMET model uses the ERA-Interim reanalysis \citep{dee2011era} and the JULES model uses a bias-corrected version of ERA-Interim, WFDEI (WATCH-Forcing-Data-ERA-Interim) \citep{weedon2014wfdei}.
Each simulation scenario is run for 30 years over the historic weather variability for the period of 1981-2010.
Simulations are run for both rainfed and irrigated crops for a parameter sweep across a variety of climate and management input values (Table \ref{table:inputs}) for atmospheric CO$_2$, Temperature, precipitation (Water), and applied Nitrogen (hereafter CTWN).
...
Emulation involves fitting individual regression models for each crop, simulation model, and 0.5 degree geographic pixel from the GGCMI Phase II dataset; the regressors are the applied constant perturbations in CO$_2$, temperature, water, and nitrogen (CTWN).
...
Emulators for irrigated simulations are separate from rainfed simulations so irrigation water demand is only provided for irrigated simulations. 
Therefore, each crop model emulator included consists of six sets of regressions.
We focus on yield emulators with no growing season adaptation here for clarity. 
See supplemental for more information on irrigation water demand and adaptation cases.  

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Climatological vs. year-to-year response (temporal aggregation)}
\begin{figure}[ht]
\centering
...
A critical design choice for our emulators is concerned with temporal aggregation.
...
First, the climatological-mean yield response is much smoother than the year-to-year yield response and makes near-lossless emulation possible.
Second, the year-over-year responses to weather are generally quantitatively distinct from (and, in the case of temperature, larger than) the climatological mean responses in the GGCMI Phase II simulation output dataset. 
In the example in Figure \ref{fig:yearvclim}, responses to year-over-year temperature variations are 100\% larger than those to long-term perturbations in the baseline case, and larger still under warmer conditions, rising to nearly 200\% more in the T+6 case. 
...
The stronger year-over-year response to temperature under warmer conditions also manifests as a wider distribution of yearly yields (Figure \ref{fig:yearly}) within a 30-year simulation.
...
Note that the GGCMI Phase II training dataset does not capture one climatological factor, potential future distributional shifts in climate, because all simulations are run with fixed offsets from the historical climatology. 
...
Nonlinear yield responses to weather are well-documented in observations for temperature and water perturbations (e.g.\ \citet{Schlenker2009} for T and \citet{He2016} for W). 
The yield response also likely depends on interactions between the major inputs included in the parameter sweep.
...
We engineer additional features from the original C, T, W, and N in order to better capture these non-linearities and interactions.

We do not focus on comparing different functional forms or non-parametric models in this study, and instead choose a relatively simple parametric model with a polynomial basis function that allows for some interpretation of resultant parameter weights. 
We utilize a third order polynomial with interaction terms as the basis of our regression.
Some prior studies have used other statistical specifications in crop model emulation, e.g.\ 39 term fractional polynomial in \citet{BLANC2015} and \citet{BLANC2017}, who borrow information across space by fitting grid points simultaneously across soil region in a panel regression.
...
Although the GGCMI Phase II sampled variable space is relatively large, it is still sufficiently limited that use of the full polynomial basis function described above can be problematic in some cases. 
