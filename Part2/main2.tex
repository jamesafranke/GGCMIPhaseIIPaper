...
For this purpose we aggregate all years in each 30-year simulation of a single parameter combination, and emulate on the mean yield, using the applied CTWN anomalies as the regressors. 
We emulate aggregated means yields rather than using information from individual years because the year-over-year response can be significantly different from the forced climatological one. 
...
Distributional changes in precipitation are expected to matter less to crop yields than do those in temperature. Crops respond not to precipitation but to soil moisture, which integrates on timescales of weeks or even months \textcolor{red}{cite \cite{XX}}. 
