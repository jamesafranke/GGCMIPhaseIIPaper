...
The experiment involves nine different globally gridded crop models, each simulating multiple crops (maize, rice, soy, and spring and winter wheat) across a systematic parameter sweep of as many as 756 scenarios, each driven by a historical climate timeseries with systematic perturbations to CO$_2$, temperature, precipitation, and nitrogen application (CTWN). 
...
Temperature perturbations are applied as additive mean shifts (from -1 to +6 degrees C), precipitation as fractional multipliers (from -50\% to +30\%), and CO$_2$ and nitrogen application as fixed values. 
...
We emulate the climatological mean response, because that is the response of interest in assessments of climate change impacts. For this purpose we aggregate all years in each 30-year simulation of a single parameter combination, and emulate on the mean yield, using the applied CTWN anomalies as the regressors. We emulate aggregated means yields rather than using information from individual years because the year-over-year response can be significantly different from the forced climatological one. 
...
In the GGCMI Phase II simulation output dataset, year-over-year responses to weather are indeed quantitatively distinct from  responses to climatgological shifts. The differences are especially significant for temperature variations: year-over-year tempreature fluctionations are generally associated with much stronger yield variations than are changes in climatological means. 
This phenomenon is illustrated in Figure \ref{fig:yearvclim}, which shows irrigated (left) and rainfed (right) maize in Iowa. We show the climatological mean response across temperature and precipitation shifts, respectively, and include the 30 individual years from selected scenarios. In the baseline climate, yields are three times as sensitive to year-over-year temperature fluctuations as they are to climatological  temperature shifts (1.9 vs.\ 0.6 tons ha$^{-1}$/K); both responses rise slightly at warmer temperatures (Figure \ref{fig:yearvclim}, left). %Furthermore, the year-over-year response is nonlinear with baseline climate: while the climatological response is nearly the same in T+0 to T+6 scenarios, evolving only from XX-XX tons ha$^{-1}$/K, the year-over-year response rises by over a quarter, from 1.9 to 2.5 tons ha$^{-1}$/K. 
Yield sensitivity to precipitation, on the other hand, is similar for both year-over-year and climatological changes, though both are strongly nonlinear, with losses becoming more severe with increasing dryness  (Figure \ref{fig:yearvclim}, right). 
...
Year-over-year and climatological responses can differ for many reasons including memory in the crop model and lurking covariates, but the most likely reason is that short-term temperature flucutations are associated with different associated distributions of daily growing-season daily weather \citep[e.g.][]{Ruane2016}. (In the GGCMI Phase II simulations, climate perturbations include no change in underlying distributions.) 
Distributional changes in precipitation are expected to matter less to crop yields than do those in temperature. Crops respond not to precipitation but to soil moisture, which integrates on timescales of weeks or even months \cite{XX}. Previous studies have suggested that in all but very arid regions, yields are not sensitive to how precipitation is distributed within a month \citep{Glotter14}.
...
then you close by saying the GGCMI dataset lets you use a relatively simple form that converges well.
...
\textbf{JULES}     & 3.99  & \textcolor{red}{20.1}  & 2.89  & \textcolor{red}{9.90}  & NA     \\ \hline
\textbf{GEPIC}     & 2.97  & 0.66  & 2.17  & 1.60  & 2.04   \\ \hline
\textbf{LPJ-GUESS} & 0.57  & NA    & NA    & xxxx  & 0.18   \\ \hline
\textbf{LPJmL}     & 2.04  & 1.52  & 1.09  & 0.51  & 0.65   \\ \hline
\textbf{pDSSAT}    & 2.91  & 1.95  & 2.23  & 0.44  & 1.79   \\ \hline
\textbf{PROMET}    & 4.41  & 7.48  & 6.06  & \textcolor{red}{16.8}  & 7.07   \\ \hline
...
\begin{figure*}[ht]
    ...
    Rug plots show end of century (2090-2100 mean) values for temperature change for each CMIP-5 model (panel c. onlys) and the resultant maize production change for each crop and climate model combination (including the additional six crop model emulators not shown in full). 
