...
The complete protocol for a modeling group involves over 20,000 years of global simulated output for rainfed crop and growing-season case; or \textcolor{red}{40,000} years across all simulations. 
Because the computational demand is high, modeling groups were allowed to submit at various specified levels of participation, with the lowest level of participation consisting of 20\% of the maximum possible simulations; the mean participation level is \textcolor{red}{XX\%}.
...
\centering
   ...
   Example showing distinction between crop yield responses to year-to-year and climatological mean temperature and precipitation shifts. Figures shows
	...
	Open black circles mark climatological mean yields for each case and short colored lines the linear regression to individual yields in each case.  Bold black lines show \red{3rd order polynomial} fits through climatological mean values. 
...
\begin{figure*}[ht]
  ...
  For single-dimension damages, all other covariates held constant at baseline values (T+0K, W+0\%, C = 360ppm, and N = 200kg ha-1). 
...
\begin{figure}[ht]
    ...
    \caption{Global emulated damages for maize on currently cultivated lands for three example GGCMI Phase II models. Models not show: CARAIB, EPIC-TAMU, JULES, GEPIC, LPJ-GUESS, and PEPIC.
    Scatter points show the emulated values for each crop model using inputs from 30 climate models from the CMIP-5 archive \citep{Taylor2012} at the decadal timescale for RCP 8.5.
    ...
    Rug plots show end of century (2090-2100 mean) values for temperature change for each CMIP-5 model and resultant production change for each crop and climate model combination (including the six models not shown).
...
Finally, we show emulated climate RCP runs using projections from 30 climate models from the 

a damage function constructed from the 4D emulation, aggregated to global yield, with simulated transient climate run values shown for comparison (Figure \ref{fig:globe_em}, which shows maize on currently cultivated land). 
