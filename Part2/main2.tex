...
The dataset allows emulating the climatological mean yield response of all models with a simple polynomial in mean growing-season vaalues. %  of all models in nearly all locations.
Climatological mean yields are a central metric in climate change impact analysis; we show here that they can be captured without relying on interannual variations. %, which are quantitatively different.
	%and relevant in many analyses of effects of climate change impacts, such as in integrated climate change impact assessments.
%OLD The dataset allows emulating the climatological mean yield response without relying on interannual variations; we show that these are quantitatively different.
%Climatological mean yields are a central metric in climate change impact analysis 
	%and relevant in many analyses of effects of climate change impacts, such as in integrated climate change impact assessments.  Climatological mean yield responses can be readily captured with a simple polynomial in nearly all locations, with errors significant only in some marginal lands where crops are not currently grown. 
...
Section \ref{S:2} summarizes the rationale of the GGCMI Phase~2 CTWN-A experiment as described in full detail by \citet{franke2020ctwnexperiment} and proposes the rationale for climatological-mean yield emulation
...
Additionally, the year-to-year response can be significantly different from the forced climatological one, so we do not attempt to use information from year-to-year variability but instead emulate the aggregated mean yield in each 30-year simulation. 
...
Results of the feature selection process suggest that 11 terms can be omitted with negligible impact on emulator fidelity, producing the 23-term statistical model of Equation \ref{eqn:features_reduced} (with removed terms shown in  {\color{dark-gray} gray}).
...
These cases are far more common for regions that are either too cold or too dry to agriculture today and become viable under warming. Individual processes included in models leading to threshold behavior are varied, see model description papers referenced in Table \ref{table:models} for more details.
...
Actual simulation cases included in each training split vary by model depending on simulation model sampling and are too extensive to list in detail here.
...
less than 0.2 ton ha$^{-1}$ for all except JULES soybean simulations, which is 0.36 ton ha$^{-1}$ in the global mean.
For irrigated crops, absolute emulator errors are generally lower, but since irrigated crops experience lower yield changes across the C,T,W,N scenarios the fractional errors are similar. 
...
We first drive the LPJmL crop model (used as a representative of GGCMI models) with climate model output under the high-end RCP 8.5 scenario.
...
\begin{figure*}[ht]
    ...
    No squares are shown in panel \textbf{b.} because the regular offsets of both T and W are not directly comparable to GCM-specific changes of T and W under an RCP.
...
emulators should be used for absolute impacts projections only in combination with historical yield data (the emulator intercept provided is an uncalibrated proxy of historical mean yields).
...
The multi-model ensemble emulation method presented here allows some future model comparison efforts, including studying locations of model consensus (or lack thereof) of sensitivity to yield drivers by comparing emulator parameters, among others.
...
While we tested the importance of different dimensions across area with current cultivation, future work may wish to test if these relationships are constant across space and in regions without current cultivation.
...
This research was performed as part of the Center for Robust Decision-making on Climate and Energy Policy (RDCEP) at the University of Chicago. This is paper number 36 of the Birmingham Institute of Forest Research. 
