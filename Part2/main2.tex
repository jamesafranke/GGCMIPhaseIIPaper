...
\begin{document}
\title{The GGCMI Phase~2 emulators: global gridded crop model responses to changes in CO$_2$, temperature, water, and nitrogen (version 1.0)}

\Author[1,2]{James A.}{Franke}
...
\affil[20]{Global Systems Institute, University of Exeter, Laver Building, North Park Road, Exeter, EX4 4QE, UK}
...
We describe here the development of emulators for nine process-based crop models and five crops using output from the Global Gridded Model Intercomparison Project (GGCMI) Phase~2. 
The GGCMI Phase~2 experiment is designed with the explicit goal of producing a structured training dataset for emulator development that samples across four dimensions relevant to crop yields: atmospheric carbon dioxide (CO$_2$) concentrations, temperature, water supply, and nitrogen inputs (CTWN). 
...
\textcolor{red}{Climatological mean yields are a central metric in climate change impact analysis and relevant in many analyses of effects of climate change impacts, such as in integrated climate change impact assessments. }
...
\textcolor{red}{Section \ref{S:2} summarizes the rationale of the GGCMI Phase~2 CTNW-A experiment as described in full detatail by \citet{franke2020ctwnexperiment} and proposes the rationale for climatological-mean yield emulation
before Section \ref{S:3} details the emulator design.}

% Readers may want to skip to section Section \ref{S:3} for emulator details.
...
\begin{table*}[ht]
    \caption{
    Crop models included in GGCMI Phase~2 emulators and the number of CTWN-A (Carbon \textcolor{red}{dioxide}, Temperature, Water, Nitrogen, Adaptation) simulations performed for each model. 
    The maximum number is 756 for A0 (no adaptation) experiments, and 648 for A1 (maintaining growing \textcolor{red}{season} length) experiments, since T0 is not simulated under A1. 
    ...
    For clarity, three simulation models \textcolor{red}{that submitted data to the GGCMI Phase~2 experiment \citep{franke2020ctwnexperiment}} are not shown here, \textcolor{red}{as those} provided a training set too small to be used in emulation.
    }
    \label{table:models}
...
\textcolor{red}{See \citet{franke2020ctwnexperiment} for the parameter combinations included by each model.}
...
\textcolor{red}{Crop models included here are not formally calibrated, given that there is no adequate calibration target for gridded global-scale crop model simulations. 
...
% I wouldn't go deeper into this here, but we need to come back to this in the discussion
In analyses where we distinguish yields over currently cultivated land, we use the \textcolor{red}{ harvested area} masks of \citet{Portmann2010}. 
...
\textcolor{red}{The central metric in assessments of climate change impacts on crop yields is the change in multi-annual means \citep[e.g.][]{Challinor2014, Rosenzweig2014, Muller_2015_implications, zhao2016plausible} } \textcolor{blue}{need to verify and add refs}
\textcolor{red}{and many impact assessments building on projected climate change impacts on crop yields work with multi-annual yields, as their analysis frameworks require information on long-term effects rather than year-to-year variability \citep[e.g.][]{nelson2014climate, Stevanovic2016, Wiebe_2015, hasegawa2018risk}} \textcolor{blue}{add refs, maybe take from Abigail's GMD paper framing?}
\textcolor{red}{Changes in extremes or year-to-year variability are other metrics of interest, but often not explicitly considered in integrated climate change impact assessments or land-use change projections that require light-weight flexible emulators, so that} we emulate the climatological mean response.
% \textcolor{red}{because that is response of interest in long-term climate change impacts, including land-use changes and, and following with the IPCC RCP framework.}
%because that is the response of interest in assessments of climate change impacts. 
\textcolor{red}{Additionally,} the year-to-year response can be significantly different from the forced climatological one, so we do not \textcolor{red}{attempt to} use information from year-to-year variability but instead emulate the aggregated mean yield in each 30-year simulation. 
...
In the GGCMI Phase~2 simulation output dataset, year-to-year responses to weather are often quantitatively distinct from responses to climatological shifts, with the discrepancy especially strong in wheat and rice. 
The difference in behavior is illustrated in Figure \ref{fig:yearvclim}, which shows irrigated and rainfed maize and wheat in representative locations. %\strikethroug; open circles and black lines show the climatological mean response, and solid circles and colored lines the responses for the 30 individual years in individual scenarios.
...
Although models exhibit different absolute levels of error, all three agree remarkably well on feature importance, i.e.\ on which terms reduce error and which provide no predictive benefit. Agreement \textcolor{red}{is indicated by matching line slopes} in Figure \ref{fig:features}. 
...
Results of the feature selection process suggest that 11 terms can be omitted with negligible impact on emulator fidelity, producing the 23-term statistical model of Equation \ref{eqn:features_reduced} (\textcolor{red}{ with removed terms shown in  {\color{dark-gray} gray}}).
...
These cases involve yield responses that benefit strongly from inclusion of higher order carbon \textcolor{red}{dioxide} interaction terms. 
...
Finally, in Section 4.3, we assess the emulator's ability to reproduce crop yields in a more realistic future simulation driven by a climate model projection, and find that any effects of changes in climate variability not included in the GGCMI Phase~2 training set are generally small relative to the effects of mean changes.
...
The GGCMI Phase~2 protocol specified only three nitrogen levels (10, 60 and 200 kg~N y$^{-1}$ ha$^{-1}$), so a third-order fit would be over-determined but a second-order fit can result in potentially unphysical results. 
...
In this test the GGCMI Phase~2 dataset is split randomly into two parts, with 90\% of the data used to train the model and the held-out 10\% used to test the fidelity of the resulting emulator.
We calculate the root mean square error (RMSE) between emulated (predicted) and actual simulated values across the test set, repeat the process twice, and average the results of the three splits. 
\textcolor{red}{Actual simulation cases included in each training split vary by model depending on simulation model sampling and are too extensive to list in detail here.} 
...
Table \ref{table:ASE} shows the yield-change-normalized RMSE for rainfed crops in all models over currently cultivated land, both in selected major producing regions and in the global average. We include all simulations in th CTWN space and report the average error value \textcolor{red}{in Table \ref{table:ASE}}. 
Mean grid cell RMSE is below 5\% of yield changes in all cases, or in absolute terms 
less than 0.2 ton ha$^{-1}$ for all except JULES \textcolor{red}{soybean simulations}, which is 0.36 ton ha$^{-1}$ in the global mean.
For irrigated crops, absolute emulator errors are generally lower, but since irrigated crops experience lower yield changes \textcolor{red}{across the C,T,W,N scenarios} the fractional errors are similar. 
...
The goal is to assess whether effects of future changes in temperature and precipitation distributions are strong enough to compromise an emulator based on the GGCMI Phase~2 dataset.
\textcolor{red}{This is a separate (but related) point to that addressed in Section 2.2; that climatological-mean yield response is not the same as year-to-year response to mean growing season temperature.
...
We first drive the LPJmL crop model (\textcolor{red}{used as a} representative of GGCMI models) with climate model output under the high-end RCP 8.5 scenario.
...
\begin{figure*}[h!]
    ...
    \caption{
    Emulated global damage functions for the five crops over the four CTWN dimensions varied.
    Black line shows the multi-model mean and shaded area and colored lines the individual models (not all models provided all crops or simulate the N dimension, so the number of models in each case varies). 
...
\begin{figure*}[ht]
    ...
    \caption{
    Illustration of \textcolor{red}{the use of the emulator to study} the factors affecting yields in more realistic climate scenario.
    ...
    \textcolor{red}{No squares are shown in panel \textbf{b.} because the regular offsets of both T and W are not directly comparable to GCM-specific changes of T and W under an RCP.}
...
\label{S:6}
In this work we describe a new class of global gridded crop model emulators for 5 crops (maize, soy, rice and spring and winter wheat) and 9 process-based crop models, based on the GGCMI Phase~2 dataset.
The systematic parameter sampling of the GGCMI Phase~2 experiment \textcolor{red}{\citep{franke2020experiment}} allows emulating climatological-mean crop yield responses with a relatively simple statistical model and isolating long-term impacts from confounding factors that lead to different year-to-year responses. 
\textcolor{red}{While we acknowledge that the omission of addressing year-to-year variability in our emulators limits their applicability to some assessments of climate change impacts (e.g. analyses of extreme events), changes in average conditions can be well reproduced by the emulators. Given their flexibility and low computational requirements, this makes them particularly suitable for applications in integrated climate change impact assessments and projections of land-use change \citep[e.g.][]{nelson2014}.}
...
\textcolor{red}{Evaluation of emulator performance with in- and out-of-sample tests demonstrates their ability to reproduce the response patterns of the process-based models. 
By targeting the reproduction of process-based crop model behavior rather than observed yield patterns, these emulators lend themselves to study differences between crop models and address the uncertainty embedded in the crop model choice \citep{muller_global_2017} in subsequent analyses, such as in agricultural economics \citep{Stevanovic2016}. }
...
While emulators based on the GGCMI Phase~2 protocol of uniform perturbations to historical climate will not reproduce any effects of changing variability in future climate projections (any temperature variability changes or precipitation variability changes other than multiplicative mean shifts), in practice these effects appear to be small, at least on the regionally aggregated level. 

%\textcolor{red}{
%XXX NOT SURE HOW TO HANDEL THIS RECAP XX:
%Some summary points:
%\begin{itemize}
   % \item climatological-mean yield response to mean temperature and mean precipitation is distinct from the year-to-year yield response
    %\item we emulate mean climatological yields against mean growing season weather drivers (+ CO$_2$ and applied nitrogen)
   % \item the emulators can faithfully represent the process-based models in both in- and out-of-sample tests
    %\item under climate change change in the long-term, the higher-order moments of the weather distribution matter very little compared to the change in the mean
%\end{itemize}
%} % I think all of these are sufficiently represented now.

Emulators provide a powerful tools for both model comparison and impacts assessments by capturing the responses of process-based crop models in a lightweight form. 
The emulators provide over three orders of magnitude reduction in data storage: the yield output for a single crop model that simulates all GGCMI Phase~2 scenarios for 5 crops is $\sim$12.5 GB; equivalent global gridded emulator parameters are only $\sim$20 MB and allow emulation of arbitrary future scenarios. Computational requirements are nearly negligible: a thousand years of global 0.5 degree yields, i.e\ $\sim$40,000,000 individual yield projections, can be emulated in 20 seconds on a laptop computer. 
...
\textcolor{red}{Polynomial fits, while faithful within sample, quickly become non-physical outside of the tested range. For example, ecosystem-based crop models tend to have weakening damages with additional warming, which leads the third order fit in temperature to turn around and predict positive yield gains at higher temperatures outside the range tested here (-1 to 6K). Note that climate model projections for \textit{fixed} growing season temperature change sometimes exceed 6K by the end of century under high-end climate change.}
...
emulators should be used for absolute impacts projections only in combination with historical yield data \textcolor{red}{(the emulator intercept provided is an uncalibrated proxy of historical mean yields).
Third, neither growing season specification tested here (A0 and A1) accounts for potential shifts in planting days under severe climate change. Under warming, farmers may choose planting earlier or later in the season as a major adaptation pathway \citep{waha2012} or target completely different growing seasons \citep{minoli2019modeling}. This may be a significant factor, especially in the northern latitudes where agriculture may become viable in the future. More research in needed in this area.
...
The GGCMI Phase~2 dataset and emulators invite a broad range of potential future avenues of analysis.
...
\textcolor{red}{The multi-model ensemble emulation method presented here allows some future model comparison efforts, including studying locations of model consensus (or lack thereof) of sensitivity to yield drivers by comparing emulator parameters, among others.
...
The GGCMI Phase~2 dataset can be used as a testbed for examining the ability of statistical models using more detailed within-season regressors to capture both year-over-year and climatological changes, and for more systematic studies of emulation itself, including evaluation of alternate \textcolor{red}{statistical} specifications or machine learning methods.
\textcolor{red}{Feature importance in regions without current cultivation could be tested to evaluate spatial dependence of the higher-order interactions.} \textcolor{blue}{this sentence needs a little more explanation, I, at least, don't understand it...}
...
\textcolor{red}{Our emulators that have been trained on this GGCMI Phase~2 experiment data set help to 
\begin{itemize}
  \item make flexible use of the crop model simulations in follow up studies and
  \item condense model differences to a sizeable set of parameters, facilitating better entry points for identifiying causes for diverging model behavior.
\end{itemize}
With the development, evaluation, and publication of the emulator sets we hope to enable further research in these fields.}
...
This research was performed as part of the Center for Robust Decision-making on Climate and Energy Policy (RDCEP) at the University of Chicago. This is paper number 35 of the Birmingham Institute of Forest Research. 
