...
Our emulators are trained only on growing-season means, and the GGCI Phase 2 exercise involved only changes in means. We therefore seek to assess whether changes in the higher moments of temperature and precipitation distributions in a climate projection might have effects that lead to significant emulator error. Note that we are not asking whether year-over-year climate variability matters to crop yields; this point is well-established \cite{ray_climate_2015}. The question instead is whether a realistic future climate projections involves {\it changes} in variablity large enough that they compromise an emulator based on the GGCMI Phase~2 dataset. 
