...
Each simulation scenario is run for 30 years over historic weather for the period of 1981-2010.
Simulations are run for both rainfed and irrigated crops for a parameter sweep across a variety of climate and management input values (Table \ref{table:inputs}) for 4 levels of atmospheric CO$_2$, 7 levels of Temperature, 8 levels of precipitation (Water), and 3 levels of applied Nitrogen (hereafter CTWN).
In each simulation case, the temperature and/or precipitation is offset from the historical values by either an additive mean shift or a fractional multiplier.
...
Emulation involves fitting individual regression models for each crop, simulation model, and 0.5 degree geographic pixel from the GGCMI Phase II dataset; the regressors are the applied constant perturbations in CO$_2$, temperature, water, and nitrogen (CTWN) and the target is the mean-climatological yield.
...
We utilize a third order polynomial with interaction terms as the basis of our regression (Equation \ref{eqn:features_original}).
