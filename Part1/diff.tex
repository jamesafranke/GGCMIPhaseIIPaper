...
%DIF ADD main1.tex            Fri Mar 13 07:31:46 2020
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \runningtitle{The GGCMI Phase II experiment}
...
The Global Gridded Crop Model Intercomparison (GGCMI) Phase II experiment, an activity of the Agricultural Model Intercomparison and Improvement Project (AgMIP), is designed to provide a systematic parameter sweep focused on climate change factors and their interaction with overall soil fertility, to allow both evaluating model behavior and emulating model responses in impact assessment tools.
...
We present some crop yield results to illustrate general characteristics of the simulations and potential uses of the GGCMI Phase II archive. 
...
}\DIFaddend GGCMI Phase II compares simulations across a set of inputs with uniform perturbations to historical climatology,  
...
The GGCMI Phase I intercomparison was a relatively limited computational exercise, requiring yield simulations for 19 crops across a total of 310 model-years of historical scenarios, and had the participation of 14 modeling groups.
The GGCMI Phase II protocol is substantially larger, involving over 1400 individual 30-year global scenarios, or over 42,000 model-years; 12 modeling groups nevertheless participated. To reduce the computational load, the GGCMI Phase II protocol reduces the number crops to 5 (maize, rice, soybean, spring wheat, and winter wheat). 
...
\DIFaddbegin \DIFadd{Values for nitrogen application levels are intended to cover a wide range of potentials. }\DIFaddend The resulting GGCMI Phase II dataset \DIFdelbegin \DIFdel{therefore }\DIFdelend captures the distribution of crop model responses over \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{a wide }\DIFaddend range of potential future climate \DIFaddbegin \DIFadd{and management }\DIFaddend conditions.


\begin{table}[t]
  \caption{
  GGCMI Phase II input parameter levels for each dimension. Temperature and precipitation values indicate the perturbations from the historical climatology. 
...
.) 
A GGCMI Phase II perturbation case also represents a set of plausible values applied to individual grid cells, not a consistent and realistic future scenario under a global mean climate shift.
In the protocol used, temperature and precipitation perturbations are applied uniformly in space, but future changes in temperature and precipitation will not be spatially or temporally uniform.
...
An individual GGCMI Phase Ii simulation therefore represents a  possible future state that }\textit{\DIFadd{could}} \DIFadd{occur in each grid cell, but not one that would in reality occur simultaneously in all grid cells across the globe.  %DIF >  that the T+1 is 1 $^{\circ}$C warmer everywhere.
...
The GGCMI Phase II simulations are intended to be used for climate impact assessment not directly but instead as a ``training set'' for statistical emulation of each crop model. Once an emulator is constructed from the outputs described here, it can be driven with growing-season climate anomalies from nay climate model projection. 
See \mbox{%DIFAUXCMD
\cite{Franke2020} }\hspace{0pt}%DIFAUXCMD
for a demonstration of the efficacy of GGCMI Phase II emulators in reproducing crop model yields under realistic future climate scenarios.
}\DIFaddend 

\DIFdelbegin \DIFdel{Each model is run at 0.5 degree spatial resolution and covers all }\DIFdelend \DIFaddbegin \DIFadd{The area simulated in the GGCMI Phase II protocol extends considerably outside }\DIFaddend currently cultivated areas\DIFdelbegin \DIFdel{and much of the uncultivated land area}\DIFdelend \DIFaddbegin \DIFadd{, because cultivation will likely shift under climate change}\DIFaddend .
Figure \ref{fig:crop_area} \DIFdelbegin \DIFdel{, left, }\DIFdelend shows the present-day cultivated area of rainfed crops \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{(left), and model coverage (right). 
(See }\DIFaddend Supplemental Figure S1-2 that for irrigated crops.\DIFdelbegin \DIFdel{Cultivated areas are provided by the MIRCA2000 (Monthly Irrigated and Rainfed Crop Area) 
data product \mbox{%DIFAUXCMD
\citep{Portmann2010}}\hspace{0pt}%DIFAUXCMD
.
Coverage extends considerably outside }\DIFdelend \DIFaddbegin \DIFadd{) 
...
Areas are not simulated if they are }\DIFdelend \DIFaddbegin \DIFadd{, omitting  areas }\DIFaddend assumed to remain non-arable even under an extreme climate change\DIFdelbegin \DIFdel{; these regions }\DIFdelend \DIFaddbegin \DIFadd{. Regions that may be omitted }\DIFaddend include Greenland, far-northern Canada, Siberia, Antarctica, the Gobi and Sahara Deserts, and Central Australia.
...
, we here assume }\DIFdelend \DIFaddbegin \DIFadd{(The exception is that Phase II, unlike Phase I, uses }\DIFaddend identical growing seasons for rainfed and irrigated \DIFdelbegin \DIFdel{scenarios}\DIFdelend \DIFaddbegin \DIFadd{cases}\DIFaddend , to allow for direct \DIFdelbegin \DIFdel{comparability }\DIFdelend \DIFaddbegin \DIFadd{comparison }\DIFaddend of simulations along the W dimension\DIFdelbegin \DIFdel{, in which irrigation (W$_{\infty}$) is one element.
...
.
In all the GGCMI Phase II crop models, }\DIFaddend sowing dates are prescribed directly\DIFdelbegin \DIFdel{and held fixed in models, }\DIFdelend \DIFaddbegin \DIFadd{, but }\DIFaddend the length of the growing season is a product of crop phenology, which in turn is mostly driven by phenological parameters and temperature. 
...
\DIFadd{The twelve models participating }\DIFaddend in GGCMI Phase II \DIFdelbegin \DIFdel{provide 7 mandatory output variables if available (Table \ref{table:outputs}, bold}\DIFdelend \DIFaddbegin \DIFadd{are listed in Table \ref{table:models}. 
...
\begin{table*}[t]
\caption{
\DIFaddFL{Models included in GGCMI Phase II and the number of CTWN-A simulations performed. 
...
\DIFadd{Table \ref{table:models} also descibes the simulation output contibution of each model to the GGCMI Phase II archive.
...
\DIFadd{All models in GGCMI Phase II }\DIFaddend provide 30-year timeseries of annual crop yields 
...
\begin{table}[]
  \caption{Output variables, naming convention, and units in the GGCMI Phase II protocol. Items in \textbf{bold} are the mandatory minimum requirements \DIFaddbeginFL \DIFaddFL{(if model capacities allow for these outputs)}\DIFaddendFL . Other variables are optionally provided depending on availability and participating modeling groups provided between 3 (PEPIC) and 18 (APSIM-UGOE) of these optional variables.}
  \label{table:outputs}
  \begin{tabular}{lll}
    \tophline \vspace{1mm}
...
\DIFaddbegin \DIFadd{All models participating in GGCMI Phase II have be evaluated against histoircal yields and site specific experimental data.
Most models (9 of 12, all but  CARAIB, JULES, and PROMET) have been evaluated in their global setup in the GGCMI Phase I evaluation exercise \mbox{%DIFAUXCMD
...
.
}\DIFaddend Evaluating the performance of crop models in the GGCMI Phase II archive is complicated by the artificial nature of the protocol: the settings in the CTWN-A experiment design do not reflect actual conditions in the real world. 
...
Following this approach, we compare yields in the GGCMI Phase II baseline simulations with detrended historical yields from the Food and Agriculture Organization of the United Nations \citep{FAOSTAT} by calculating the Pearson product moment correlation coefficient over 26 years of yield.
...
For example, global correlation coefficients \DIFdelbegin \DIFdel{for maize }\DIFdelend in Phase I and Phase II\DIFaddbegin \DIFadd{, respectively, }\DIFaddend are 0.89 and 0.74 \DIFdelbegin \DIFdel{, respectively; for wheat }\DIFdelend \DIFaddbegin \DIFadd{for maize, }\DIFaddend 0.67 and 0.64 \DIFdelbegin \DIFdel{, and for soybeans }\DIFdelend \DIFaddbegin \DIFadd{for wheat, and }\DIFaddend 0.64 and 0.59 \DIFdelbegin \DIFdel{.
(Compare to \mbox{%DIFAUXCMD
\citet{muller_global_2017} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{for soybeans.
(Phase I values are from }\DIFaddend Figures 1--4 and \DIFdelbegin \DIFdel{6.}\DIFdelend \DIFaddbegin \DIFadd{6 in \mbox{%DIFAUXCMD
\citet{muller_global_2017}}\hspace{0pt}%DIFAUXCMD
.}\DIFaddend )  
...
The responses highlighted here are qualitatively similar across all crops included in this study (Supplemental Figures S5 - \DIFdelbegin \DIFdel{S8}\DIFdelend \DIFaddbegin \DIFadd{S9 for all simulated area and S10 - S14 for cultivated area only}\DIFaddend ). 
...
See Supplemental Figures S15 for example reponses to temperature change in the A1 sceaniros. 
Supplemental Figures S16 and S17 show the nitrogen and dimension and a irrigation water demand response example.
}

\DIFaddend \section{Discussion and Conclusions} 
\label{S:5}
The GGCMI Phase II experiment provides a database designed to allow detailed study of crop yields in process-based models under climate change. 
...
\citep[e.g.][]{Rosenzweig2014}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , the use of systematic input parameter variations in GGCMI Phase II (with up to 756 scenarios) allows not only comparing yield sensitivities to changing climate and management inputs but also evaluating the complex interactions between important driving factors: CO$_2$, temperature, water supply, and applied Nitrogen. 
...
In validation tests of simulations of the historical scenario, year-over-year correlations in modeled and actual country-level yields are similar to those of GGCMI Phase I. 
...
GGCMI Phase II is intended as a study of model-projected changes under broad climate change, which may not be as sensitive to the adjustments needed to reproduce present-day yields. 
...
The second major caveat is that no individual GGCMI Phase II simulation is itself a realistic future yield projection. 
...
\DIFaddbegin \DIFadd{As such it also remains challenging to evaluate model performance as the actual growing conditions have to be interpolated between the uniform input levels used in the }\DIFaddend GGCMI Phase II \DIFaddbegin \DIFadd{setup. 
GGCMI Phase II }\DIFaddend simulation results can be used for impacts projection, but only with the construction of an emulator of crop yield response to climatological changes, that is then driven by a realistic climate projection.
\DIFdelbegin \DIFdel{However, note that }\DIFdelend \DIFaddbegin \DIFadd{Note that while }\DIFaddend the experiment does not sample over potential changes in the higher-order moments in the distributions in temperature and precipitation\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{, we shown these higher-order changes matter little for an impact assessment under a transient climate run such as an RCP from CMIP-5 when conducted with emulators developed from the GGCMI Phase II simulations \mbox{%DIFAUXCMD
\citep{Franke2020}}\hspace{0pt}%DIFAUXCMD
.
A final caveat is that since simulations are run off-line and therefore do not contain any feedbacks between the atmosphere and land such as irrigation impacts on humidity.
}\DIFaddend 

\DIFaddbegin \DIFadd{As most simulation studies on climate change impacts on crop yields, the simulations here are driven by external weather data, i.e. feedbacks from plant growth (e.g. albedo, evapotranspiration) and management (e.g. irrigation) on the atmosphere are not included here. 
While we acknowledge that these mechanisms exist (e.g. Decker et al. 2017 see https://iopscience.iop.org/article/10.1088/1748-9326/aa65a6/pdf), these feedbacks need to be explored in more specialized analyzes, as they require an Earth-sytem model analysis framework, which is in conflict with the design of systematic offsets in climatic drivers.
}

\DIFaddend We expect that the GGCMI Phase II simulations will yield multiple insights in future studies. 
