...
%DIF ADD main1.tex            Thu Mar 12 18:24:26 2020
...
Process-based models also allow for the global gridded simulations needed for understanding the global dynamics of agricultural trade, including cultivation area changes and crop selection switching under climate change \citep{rosenzweig2018,ruane2018} because global market mechanisms may strongly modulate climate change impacts \citep{Stevanovic2016,hasegawa2018risk}. \DIFdelbegin \DIFdel{Global }\DIFdelend \DIFaddbegin \DIFadd{While global model applications imply large uncertainty due to poor input data quality and no ability for concise model calibration, global }\DIFaddend crop model experiments are needed for systematic climate change assessments \citep{muller_global_2017} \DIFaddbegin \DIFadd{where inter-regional differences need to be considered as in economic assessments \mbox{%DIFAUXCMD
\citep[e.g.][]{Wiebe_2015,Nelson2014}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
...
can more realisticly inform climate change impact assessments, an analysis of model sensitivities and response types is hampered by such non-systematic experiments. 
...
\DIFaddbegin \DIFadd{Note that both irrigated and fully rainfed systems are simulated with global coverage, independent of their actual occurrence. 
This allows for better analysis of model sensitivities and responses to water limitations. 
In the GGCMI Phase II experiment, irrigated systems are simulated using rainfed growing seasons so that the 2 simulation sets are directly comparable even though in reality irrigated growing seasons can be in different periods of the year than rainfed growing seasons \mbox{%DIFAUXCMD
\citep{Portmann2010}}\hspace{0pt}%DIFAUXCMD
.
}\DIFaddend %For irrigated simulations, models are set to apply near-perfect irrigation to keeps soils wet throughout the entire growing period, with no limitations in water supply. 
...
\DIFaddbegin \DIFadd{Nitrogen application levels are intended to cover a wide range of potentials across all crops, regions, and futures but may be unrealistic in many cases such as any N application to soybeans or values of 200 kg ha$^-1$ on crops in presently less-industrialized areas (200 kg ha$^-1$ may also be too low in some present cases).
}\DIFaddend 
...
Likewise, temperature and precipitation perturbations are applied uniformly in space so that the T+1 is 1 $^{\circ}$C warmer everywhere.
Neither GGCMI phase II applied change to the climatology is very likely under a realistic climate change, where the modifications to temperature distributions will be non-uniform in space and across the year.
...
\DIFaddbegin \DIFadd{The parametrization of growing seasons can have strong effects on simulated yields \mbox{%DIFAUXCMD
\citep{muller_global_2017, Jag2018} }\hspace{0pt}%DIFAUXCMD
and it is essential for comparability of simulation results to harmonize between models.
}\DIFaddend Given that temperature varies between years, individual years can vary from the harmonization target.
...
\DIFaddbegin \DIFadd{As such, each model is run with seven different cultivar parameter sets in the A1 simulations (one per warming level) and the cultivar parameters in A0 and A1 are identical for T=0. 
...
If }\DIFaddend models do not \DIFdelbegin \DIFdel{output }\DIFdelend \DIFaddbegin \DIFadd{compute specific outputs, such as }\DIFaddend the anthesis date\DIFdelbegin \DIFdel{.)
}\DIFdelend \DIFaddbegin \DIFadd{, these models can still participate in the GGCMI Phase II experiment. All models capable of computing the mandatory outputs are expected to also deliver these outputs..
...
\DIFadd{The models participating in GGCMI phase II are listed in Table \ref{table:models}. 
Models differ substantially in structure and parameterization and can be separated into two broad categories, site-based (field-scale) and global Ecosystem models
Site based models include APSIM, pDSSAT, and the EPIC family of models and LPJmL, LPJ-GUESS, PROMET, CARAIB, ORCHIDEE, and JULES are global ecosystem models.
Different models employ a variety of approaches for the core modules such as primary production or evapotranspiration.
Most models utilize either a light use efficiency or photosynthetic approach for primary production and the Priestley-Taylor, Penman-Monteith or Hargreaves schema for evapotranspiration. However, JULES and PROMET utilize a land surface model approach to calculate water vapor fluxes for evapotranspiration.
Models that share a common genealogy such as the EPIC family of models can still have very different schema for these core components.
For example EPIC-TAMU uses Penman-Monteith for evapotranspiration and EPIC-IIASA uses the Hargreaves schema.
Most models utilize either the Harmonized World Soil Database (HWSD) from the FAO \mbox{%DIFAUXCMD
...
More details on spin-up, calibration other than growing season, residue management, and irrigation rules can be found in Supplemental Table S1.
}
...
For example, global correlation coefficients for maize in Phase I and Phase II are 0.89 and 0.74 \DIFdelbegin \DIFdel{, respectively; for wheat }\DIFdelend \DIFaddbegin \DIFadd{for maize, }\DIFaddend 0.67 and 0.64 \DIFdelbegin \DIFdel{, and for soybeans }\DIFdelend \DIFaddbegin \DIFadd{for wheat, and }\DIFaddend 0.64 and 0.59 \DIFdelbegin \DIFdel{.
(Compare to \mbox{%DIFAUXCMD
\citet{muller_global_2017} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{for soybeans, respectively (Phase I values are from }\DIFaddend Figures 1--4 and \DIFdelbegin \DIFdel{6.)}\DIFdelend \DIFaddbegin \DIFadd{6 in \mbox{%DIFAUXCMD
\citet{muller_global_2017}}\hspace{0pt}%DIFAUXCMD
).  
}\DIFaddend Differences in fidelity between regions and crops exceed differences between models: 
that is, Figure \ref{fig:simulation_val}\DIFaddbegin \DIFadd{(c)--\ref{fig:simulation_val}(f) }\DIFaddend shows more color similarity in horizontal than vertical bars. 
...
\DIFaddbegin \DIFadd{Yield reductions are due in part to shortenting of the growing season under warming and in part to the direct effects of higher temperature.
}\DIFaddend Models show some increases in high mountain regions that are currently cold-limited.
...
We illustrate a few of these relationships in Figures \ref{fig:maizerice}-\ref{fig:soywheat}, choosing crops and factors whose effects are reasonably well understood. 
...
\DIFaddbegin \DIFadd{XXX - Something about irrigation water demand and the A1 dimension results???
}

\DIFadd{See Supplemental Figures S8-10 for example reponses in the nitrogen and A1 dimensions and for a irrigation water demand example.
}
...
XXX-From Christoph, suggest move to Section 4.1 However, the models have mostly been evaluated in their global setup in the GGCMI Phase I evaluation exercise \mbox{%DIFAUXCMD
...
and all models have been evaluated in the course of their development, including evaluations against site-specific experimental data.
