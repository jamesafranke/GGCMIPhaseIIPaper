...
%DIF ADD main1.tex            Thu Mar 12 21:25:19 2020
...
For example, modeled yields show robust decreases to warmer temperatures in almost all regions, with a nonlinear dependence that indicates yields in warmer baseline locations have greater temperature sensitivity. 
...
Global crop model experiments are needed for systematic climate change assessments \citep{muller_global_2017} \DIFaddbegin \DIFadd{which must consider inter-regional differences need to be considered as in economic assessments \mbox{%DIFAUXCMD
...
\DIFaddbegin \DIFadd{Climate change does not only affect overall temperature and precipitation levels, but also their distribution within growing seasons \mbox{%DIFAUXCMD
\citep[e.g.][]{Holmes2016}}\hspace{0pt}%DIFAUXCMD
. 
While crop model simulations based on climate projections from climate models as e.g. from the CMIP \mbox{%DIFAUXCMD
\citep[Coupled Model Intercomparison Project, see][]{Eyring2016} }\hspace{0pt}%DIFAUXCMD
archive as used by \mbox{%DIFAUXCMD
\citet[e.g.][]{Rosenzweig2014} }\hspace{0pt}%DIFAUXCMD
can more realistically inform climate change impact assessments, an analysis of model sensitivities and response types is hampered by such non-systematic experiments. 
The GGCMI Phase II experiment is designed for scrutinizing model behavior and sensitivities as well as for crop model emulator design and does not fully replace crop model simulations with sophisticated climate scenarios. 
}\DIFaddend 
...
\DIFaddbegin \DIFadd{Values of climate variable perturbations are selected to represent reasonable ranges for changes over the medium term (to 2100) under business-as-usual emissions. 
Values for nitrogen application levels are intended to cover a wide range of potentials. 
Because the same values are applied across all crops and regions, many individual cases will not be realistic e.g.\ any N application to soybeans or high  in presently less-industrialized areas .
The resulting GGCMI Phase II dataset therefore captures the distribution of crop model responses over the range of potential future climate and management conditions.
 }

\DIFaddend In irrigated simulations, crops are assumed to have no water constraints, i.e. all crop water requirements are fulfilled regardless of local water supply limitations.
...
\DIFdelbegin \DIFdel{Values of climate variable perturbations are selected to represent reasonable ranges for changes over the medium term (to 2100) under business-as-usual emissions. 
The resulting }\DIFdelend \DIFaddbegin \DIFadd{Note that both irrigated and fully rainfed systems are simulated with global coverage, independent of their actual occurrence. 
This allows for better analysis of model sensitivities and responses to water limitations. 
In the }\DIFaddend GGCMI Phase II \DIFdelbegin \DIFdel{dataset therefore captures the distribution of crop model responses over the range of potential future climate conditions}\DIFdelend \DIFaddbegin \DIFadd{experiment, irrigated systems are simulated using rainfed growing seasons so that the two simulation sets are directly comparable even though in reality irrigated growing seasons can be in different periods of the year than rainfed growing seasons \mbox{%DIFAUXCMD
...
\DIFaddbegin \DIFadd{Temperature and precipitation perturbations from historical climatology are applied within the growing season so that the T+1 simulations represent a 1 $^{\circ}$C warmer growing season.
For example, winters may likely warm faster than summers \mbox{%DIFAUXCMD
\citep[e.g.][]{Haugen2018} }\hspace{0pt}%DIFAUXCMD
so that a 1 $^{\circ}$C warmer year under climate change may be only some fraction of a degree warmer during the growing season.
Likewise, temperature and precipitation perturbations are applied uniformly in space in GGCMI Phase II so that the T+1 is 1 $^{\circ}$C warmer everywhere.
Higher latitudes may warm faster than lower latitudes \mbox{%DIFAUXCMD
...
Changes in temperature and precipitation under realistic climate change will likewise not be spatially or temporally uniform, so in order to use the GGCMI Phase II simulations for climate impact assessment, one must emulate the Phase II simulation outputs and then drive the emulator with growing season climate anomalies calculated from a climate model RCP run within the growing season in each location.
See \mbox{%DIFAUXCMD
\cite{Franke2020} }\hspace{0pt}%DIFAUXCMD
for more details on GGCMI emulator construction for impact assessment.
}

\DIFaddend Each model is run at 0.5 degree spatial resolution and covers all currently cultivated areas and much of the uncultivated land area. 
Figure \ref{fig:crop_area}, left, shows the present-day cultivated area of rainfed crops and Supplemental Figure S1-2 that for irrigated crops. 
...
In contrast to GGCMI Phase I \citep{Elliott2015}, we here assume identical growing seasons for rainfed and irrigated scenarios, to allow for direct comparability of simulations along the W dimension, in which irrigation (W$_{\infty}$) is one element. (See Table \ref{table:inputs}.) 
...
Modelers are asked to adjust the phenological parameters so that growing season length of the baseline scenario (C=360, T=0, W=0) on average matches the harmonization target. 
\DIFdelbegin \DIFdel{Given }\DIFdelend \DIFaddbegin \DIFadd{Harmonizing between models is important because the parametrization of growing seasons can have strong effects on simulated yields \mbox{%DIFAUXCMD
\citep{muller_global_2017, Jag2018}}\hspace{0pt}%DIFAUXCMD
.
However, given }\DIFaddend that temperature varies between years, individual years can vary from the harmonization target.
...
The one exception to the harmonization protocol described above involves CARAIB, which \DIFdelbegin \DIFdel{for technical reasons }\DIFdelend kept their own growing season specifications rather than tuning to standard lengths.
...
For these simulations, modelers adjust parameters \DIFaddbegin \DIFadd{in each temperature scenario }\DIFaddend to hold growing season length approximately constant\DIFdelbegin \DIFdel{across the different warming scenarios}\DIFdelend . 
(CARAIB simulations follow the same principle, fixing growing season length at their baseline levels.) 
\DIFaddbegin \DIFadd{That is, each model is run with seven different cultivar parameter sets in the A1 simulations (one per warming level) and the cultivar parameters in A0 and A1 are identical for T=0. 
...
For each scenario, 0.5 degree grid cell and crop, models provide 30-year timeseries of annual crop yields in units of tons ha$^{-1}$ year$^{-1}$, as well as \DIFaddbegin \DIFadd{all available variables of the following seven: }\DIFaddend total aboveground biomass yield\DIFdelbegin \DIFdel{; }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend the dates of planting, anthesis \DIFdelbegin \DIFdel{, and maturity; }\DIFdelend \DIFaddbegin \DIFadd{and maturity, }\DIFaddend applied irrigation water in irrigated scenarios\DIFdelbegin \DIFdel{; and total evapotranspirtation. 
(Note that several }\DIFdelend \DIFaddbegin \DIFadd{, and total evapotranspiration. 
Note that some }\DIFaddend models do not \DIFdelbegin \DIFdel{output the anthesis date.
)
}\DIFdelend \DIFaddbegin \DIFadd{compute some mandatory outputs, e.g. CARAIB does not compute the thesis date.
}\DIFaddend Besides these mandatory 7 data products, the protocol requests any or all of 18 optional additional output variables (Table \ref{table:outputs}, plain text).
...
\DIFadd{The twelve models participating in GGCMI Phase II are listed in Table \ref{table:models}. 
...
Site based models include APSIM, pDSSAT, and the EPIC family of models and ecosystem models include LPJmL, LPJ-GUESS, PROMET, CARAIB, ORCHIDEE, and JULES.
Models employ a variety of approaches for the core modules such as primary production or evapotranspiration.
For completes details including on spin-up, calibration other than growing season, residue management, and irrigation rules, see Supplemental Table S1.
...
Notes that models that share a common genealogy may still use different schemes for evapotranspiration.
For example EPIC-TAMU uses the Penman-Monteith and EPIC-IIASA uses the Hargreaves scheme.
For soils most models utilize either the Harmonized World Soil Database (HWSD) from the FAO \mbox{%DIFAUXCMD
...
\DIFdelbegin \DIFdel{The simulation output contributions of the 12 crop models }\DIFdelend \DIFaddbegin \DIFadd{Table \ref{table:models} also descibes the simulation output contibution of each model }\DIFaddend to the GGCMI Phase II archive\DIFdelbegin \DIFdel{are described in Table \ref{table:models}}\DIFdelend .
...
We expect that the GGCMI Phase II simulations will yield multiple insights in future studies. 
