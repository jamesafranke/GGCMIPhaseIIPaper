...
The Global Gridded Crop Model Intercomparison (GGCMI) Phase II experiment, an activity of the Agricultural Model Intercomparison and Improvement Project (AgMIP), is designed to provide a systematic parameter sweep across critical interacting factors, to allow both evaluating model behavior and emulating their responses in impact assement tools.
...
While 10 of the 12 models participating in GGCMI Phase II use the AgMERRA historical daily climate inputs, two models require sub-daily input data or longwave radiation and use alternative baseline climate inputs. 
PROMET uses the ERA-Interim reanalysis \citep{dee2011era} in order to incorporate longwave radiation, and JULES uses a version of ERA-Interim, the 3-hour WFDEI (WATCH-Forcing-Data-ERA-Interim) \citep{weedon2014wfdei} in which the ERAI is adjusted following the procedures used on the WATCH hydrological dataset, improving the precipitation.
...
However, these differences are relatively small compared to the perturbations tested in the protocol.
...
The second set, (``A1'', with adaptation) holds the length of the growing season fixed at that of the baseline climate scenario. T
hese simulations roughly capture the case in which adaptive crop cultivar choice maintain the growing season length so that crops reach maturity at the same time as do current varieties under the current temperature regime. 
...
Modeling groups did not all implement the full protocol described above: given the substantial computational requirements, 
...
\label{table:models}
	\begin{tabular}{p{6cm} p{1cm} p{1cm} p{1cm} p{1cm} p{1cm} p{1cm} p{1.9cm}}
        ...
        {\textbf{pDSSAT},       \citet{Elliott2014b, JONES2003235}} & {X} & {X} & {X} & {X} & {X} & {X} & {756 / 756}\\ \middlehline
...
\textcolor{red}{NEED INTRO SENTENCE}
