...
First, in historical simulations, year-over-year correlations in modeled and actual country-level yields are generally good in most cases. No single model dominates performance globally with each model providing best-in-class performances in at least some areas/crops.
...
When weighted by currently cultivated grid-cells only, the ensemble median response almost always outweighs the spread across models.
Third, interactions between major yield drivers (temperature and precipitation in Figure \ref{fig:maizerice}, precipitation and CO$_2$ in Figure \ref{fig:soywheat}) generally follow expectations, and illustrate the power of the experimental setup.
