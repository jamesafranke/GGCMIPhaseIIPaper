...
The experimental protocol consists of 9 levels for water supply perturbations, 7 for temperature, 4 for CO$_2$, and 3 for applied nitrogen, for a total of 756 simulations (Table \ref{table:inputs}), 672 for rainfed agriculture and an additional 84 for irrigated (W$_{\infinity}$). 
...
Given that the irrigated scenario (W$_{\infinity}$) is one element of the water supply levels, irrigated simulations use the same growing seasons and areas as all other simulations.
...
Irrigated (W$_{\infinity}$) simulations assume the maximum beneficial levels of water. 
Bold font indicates the `baseline' or historical level for each dimension. One model provided simulations at the T + 5 level.
  ...
  \begin{tabular}{lcc} 
      ...
      {} & 10, 20, 30, (and W$_{\infinity}$) & {} \\ \middlehline \vspace{1mm}
...
In contrast to GGCMI Phase I \citep{Elliott2015}, we here assume identical growing seasons for rainfed and irrigated scenarios, to allow for direct comparability of simulations along the W dimension, in which irrigation (W$_{\infinity}$) is one element. (See Table \ref{table:inputs}.) 
...
\begin{table}[]
  ...
  \begin{tabular}{lll}
    ...
    \textbf{Evapotranspiration (growing season sum)} & \textbf{etransp\_<crop>}   & \textbf{mm yr$^{-1}$ (W$_{\infinity}$ scenarios only)}\\ \middlehline
...
GGCMI Phase II is intended as a study of model-projected changes, \textcolor{red}{which are not sensitive to the adjustments needed to reproduce present-day yields}. 
%GGCMI Phase II is intended as a study of model-projected changes, in a systematic manner. 
Note however that the models used in this exercise cannot simulate some potential climate-related yield changes: those due to factors such as pests, disseases, and weeds.
...
GGCMI Phase II simulation results can be used for impacts projection, but only with the construction of an emulator of crop yield response to climatological changes, that is then driven by a realistic climate projection. and those due to potential changes in higher-order moments in the distributions in temperature and precipitation. 
