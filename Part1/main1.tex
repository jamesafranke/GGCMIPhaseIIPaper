...
Global crop model experiments are needed for systematic climate change assessments \citep{muller_global_2017} which must consider inter-regional differences need to be considered as in economic assessments \citep[e.g.][]{Nelson2014, Wiebe_2015}.
...
In the GGCMI Phase II experiment, irrigated systems are simulated using rainfed growing seasons so that the two simulation sets are directly comparable even though in reality irrigated growing seasons can be in different periods of the year than rainfed growing seasons \citep{Portmann2010}.
...
Higher latitudes may warm faster than lower latitudes \citep[e.g.][]{Hansen1997} so that a 1 $^{\circ}$C change in the growing season in Canada, for example, may not equate to a 1 $^{\circ}$C global mean temperature rise.
...
Harmonizing between models is important because the parametrization of growing seasons can have strong effects on simulated yields \citep{muller_global_2017, Jag2018}.
However, given that temperature varies between years, individual years can vary from the harmonization target.
...
For these simulations, modelers adjust parameters in each temperature scenario to hold growing season length approximately constant. 
(CARAIB simulations follow the same principle, fixing growing season length at their baseline levels.) 
That is, each model is run with seven different cultivar parameter sets in the A1 simulations (one per warming level) and the cultivar parameters in A0 and A1 are identical for T=0. 
...
For each scenario, 0.5 degree grid cell and crop, models provide 30-year timeseries of annual crop yields in units of tons ha$^{-1}$ year$^{-1}$, as well as all available variables of the following seven: total aboveground biomass yield, the dates of planting, anthesis and maturity, applied irrigation water in irrigated scenarios, and total evapotranspiration. 
Note that some models do not compute some mandatory outputs, e.g. CARAIB does not compute the thesis date.
...
The twelve models participating in GGCMI Phase II are listed in Table \ref{table:models}. 
...
For soils most models utilize either the Harmonized World Soil Database (HWSD) from the FAO \citep{FAOSOIL2008} or the ISRIC-WISE database \citep{BatjesL2008} or a derivation thereof.
...
Table \ref{table:models} also descibes the simulation output contibution of each model to the GGCMI Phase II archive.
...
All models participating in GGCMI Phase II have be evaluated against histoircal yields and site specific experimental data.
Most models (9 of 12, all but  CARAIB, JULES, and PROMET) have been evaluated in their global setup in the GGCMI Phase I evaluation exercise \citep{muller_global_2017} or have later used that online tool for the evaluation of subsequent model versions \citep[e.g.][]{von_Bloh_implementing_2018}.
...
For example, global correlation coefficients in Phase I and Phase II, respectively, are 0.89 and 0.74 for maize, 0.67 and 0.64 for wheat, and 0.64 and 0.59 for soybeans.
(Phase I values are from Figures 1--4 and 6 in \citet{muller_global_2017}.)  
...
The responses highlighted here are qualitatively similar across all crops included in this study (Supplemental Figures S5 - S9 for all simulated area and S10 - S14 for cultivated area only). 
