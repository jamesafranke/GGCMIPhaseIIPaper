...
%\textcolor{red}{Explain what WFDEI does and how it's constructed. Then a sentence about differences in temp and precip absolutes and variability between these data sets.? Did you check how much this matters to yields? Are you assuming that baseline doesn't matter and you'll still capture changes well?}
...
Of the 12 models submitting data, 6 followed the \textit{full} protocol; these are marked with italic text in Table \ref{table:models}. However, note that two of these models (CARAIB and JULES) cannot represent nitrogen effects explicitly and so do not sample over the the nitrogen dimension. Two models followed \textit{high} with minor modifications (GEPIC adding an additional T level and PROMET omitting the intermediate N level). One model (PEPIC) followed \textit{mid} but included an additional C level. Three models approximately followed \textit{low} with APSIM-UGOE and EPIC-IIASA providing some additional levels and ORCHIDEE-crop omitting a TxW combination.  
...
To illustrate the properties of the GGCMI Phase II model simulations, we provide both an assessment of model performance by comparing with observed yields, and selected case studies showing the spread of model responses to climate and management inputs.
...
%We focus on yield data, as this data set if presumably of greatest interest and yield data are the more important and best evaluated outputs of GGCMs.
...
\centering
  ...
  \caption{Illustration of the spatial pattern of potential yields (left) and yield changes (right) in the GGCMI Phase II ensemble, for three major crops: maize, soybean and rice. Left column (a) shows multi-model mean climatological potential yields for the baseline scenario for (top--bottom) rainfed maize, soybean, and rice. 
   ...
   See Figure SXX in the supplemental for spatial coverage of simulations for the T+4 case.}
...
For example, three models in Phase II show almost all 0 (ton ha-1) yields north of 45$^o$ for the historical period and steep increases in yields under a 4 degree warming even for areas currently under cultivation (\ref{fig:highlat}, left panel). 
...
Models agree more about the unsuitability of soybeans in locations north of 45$^o$ where it is not currently cultivated, but the low baseline yields result in large and unstable fractional changes under warming.
...
\label{A:1}
Simulation yield output data sets can be found at the DOIs located in table \ref{table:dataloc}. 
