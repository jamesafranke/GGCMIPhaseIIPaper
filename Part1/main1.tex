...
Values for nitrogen application levels are intended to cover a wide range of potentials. The resulting GGCMI Phase II dataset captures the distribution of crop model responses over a wide range of potential future climate and management conditions.
...
An individual GGCMI Phase Ii simulation therefore represents a  possible future state that \textit{could} occur in each grid cell, but not one that would in reality occur simultaneously in all grid cells across the globe.  % that the T+1 is 1 $^{\circ}$C warmer everywhere.
...
See \cite{Franke2020} for a demonstration of the efficacy of GGCMI Phase II emulators in reproducing crop model yields under realistic future climate scenarios.

The area simulated in the GGCMI Phase II protocol extends considerably outside currently cultivated areas, because cultivation will likely shift under climate change.
Figure \ref{fig:crop_area} shows the present-day cultivated area of rainfed crops (left), and model coverage (right). 
(See Supplemental Figure S1-2 that for irrigated crops.) 
...
To reduce the computational burden, the protocol requires simulation over only 80\% of Earth land surface area, omitting  areas assumed to remain non-arable even under an extreme climate change. Regions that may be omitted include Greenland, far-northern Canada, Siberia, Antarctica, the Gobi and Sahara Deserts, and Central Australia.
...
(The exception is that Phase II, unlike Phase I, uses identical growing seasons for rainfed and irrigated cases, to allow for direct comparison of simulations along the W dimension.) % (See Table \ref{table:inputs}.) 
...
In all the GGCMI Phase II crop models, sowing dates are prescribed directly, but the length of the growing season is a product of crop phenology, which in turn is mostly driven by phenological parameters and temperature. 
...
(The one exception to the harmonization protocol described above involves CARAIB, whose team kept their own growing season specifications rather than tuning to standard lengths.)
Two aspects of this procedure should be noted. First, the target growing seasons used in GGCMI Phase II are crop- and location-specific.
...
We denote simulations that allow these future changes as ``A0'' experiments, where 0 denotes ``no adaptation''. 
 The GGCMI Phase II protocol includes a second set of experiments, ``A1'', that assume that future cultivars are modified to adjust to changes along the T dimension in the CTWN experiment.  %To account for potential adaptation in crop cultivars,
...
All models in GGCMI Phase II provide 30-year timeseries of annual crop yields 
