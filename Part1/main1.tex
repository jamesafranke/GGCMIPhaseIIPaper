...
%A GGCMI Phase II perturbation case also %represents a set of plausible values applied to individual grid cells, does not represent consistent and realistic future scenario under a global mean climate shift.
In the GGCMI Phase II protocol, temperature and precipitation perturbations are applied uniformly in space, but future changes in temperature and precipitation will not be spatially or temporally uniform.
...
A GGCMI Phase II simulation therefore represents a possible future state that \textit{could} occur in each grid cell, but not one that would in reality occur simultaneously in all grid cells across the globe.  % that the T+1 is 1 $^{\circ}$C warmer everywhere.
...
\cite{Franke2020} demonstrate that these effects are relatively minor and that GGCMI Phase II emulators can effectively reproduce crop model yields under realistic future climate scenarios.
