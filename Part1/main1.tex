...
\begin{figure*}[ht]
  ...
  Areas for winter and spring wheat areas are adapted from MIRCA2000 and two other sources; see text for details.  For irrigated crops, see Figure XX \ref{SXX}.
...
Temperatures for maize are very similar between data products, with ERA-Interim 0.3◦C cooler and WFDEI XX cooler/warmer. 
Differences are largest for rice, with ERA-Interim 1◦C cooler. 
These differences are still relatively small compared to the perturbations tested in the protocol.

%In general, temperatures are very similar between data products, with ERA-Interim being about 1.0 and 0.3$^{\circ}$C cooler than AgMERRA (or WFDEI) when compared over rice and maize cultivation areas, respectively (supplemental Figure \ref{SXX}). 
%For precipitation, ERA-Interim is substantially wetter than AgMERRA (or WFDEI) over wheat cultivation area (+60mm year$^{-1}$), and is wetter in some years over rice, maize and soybean cultivation areas (supplemental Figure \ref{SXX}). 

Planting dates and growing season lengths are standardized across models, following the procedure described in \citet{Elliott2015} for the \textit{fullharm} setting. In contrast to GGCMI Phase I \citep{Elliott2015}, we here assume identical growing seasons for rainfed and irrigated scenarios, to allow for direct comparability of simulations along the W dimension, in which irrigation (W$_{\mathit{inf}}$}) is one element (See Table \ref{table:inputs}.). 
...
All participating modeling groups provided identical coverage of the CTWN parameter space for different crops, but some provided a more limited sample for scenarios with adaptation (A1).
EPIC-IIASA, JULES, and ORCHIDEE-crop provided no A1 scenarios.
...
\begin{figure*}[ht]
    ...
    \includegraphics[width=14cm]{figures/validation_bottom.png}
    ...
    %(models that did not simulate different nitrogen levels are analogous to a high nitrogen application level). 
...
%For example, soybeans yields on all grid cells north of 45$^\circ$ in the PROMET and LPLmL models are very different in historical simulations -- mostly zero in the PROMET model, which considers present-day temperatures too cold for soy, but averaging 2 tons ha$^{-1}$ in LPJmL -- but become more similar in the T+4 scenario (to mean of \textcolor{red}{XX and XX} tons ha$^{-1}$). 
%Warming increases high-latitude soybeans yields in PROMET as land becomes suitable for cultivation, but yields in LPJmL actually shift down slightly (Figure \ref{fig:highlat}). 
