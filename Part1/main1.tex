...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\runningtitle{The GGCMI Phase II experiment}
...
Process-based models also allow for the global gridded simulations needed for understanding the global dynamics of agricultural trade, including cultivation area changes and crop selection switching under climate change \citep{rosenzweig2018,ruane2018} because global market mechanisms may strongly modulate climate change impacts \citep{Stevanovic2016,hasegawa2018risk}. While global model applications imply large uncertainty due to poor input data quality and no ability for concise model calibration, global crop model experiments are needed for systematic climate change assessments \citep{muller_global_2017} where inter-regional differences need to be considered as in economic assessments \citep[e.g.][]{Wiebe_2015,Nelson2014}.
...
Climate change does not only affect overall temperature and precipitation levels, but also their distribution within growing seasons \citep[e.g.][]{Holmes2016}. 
While crop model simulations based on climate projections from climate models as e.g. from the CMIP \citep[Coupled Model Intercomparison Project, see][]{Eyring2016} archive as used by \citet[e.g.][]{Rosenzweig2014} can more realisticly inform climate change impact assessments, an analysis of model sensitivities and response types is hampered by such non-systematic experiments. 
...
The initial GGCMI Phase I \citep{Elliott2015, muller_global_2017} compared harmonized yield simulations over the historical period, with primary goals of model evaluation and understanding sources of uncertainty (including model parameterization, weather inputs, and cultivation areas). 
...
Note that both irrigated and fully rainfed systems are simulated with global coverage, independent of their actual occurrence. 
This allows for better analysis of model sensitivities and responses to water limitations. 
In the GGCMI Phase II experiment, irrigated systems are simulated using rainfed growing seasons so that the 2 simulation sets are directly comparable even though in reality irrigated growing seasons can be in different periods of the year than rainfed growing seasons \citep{Portmann2010}.
...
For example, winters may likely warm faster than summers \citep[e.g.][]{Haugen2018} so that a 1 $^{\circ}$C warmer year under climate change may be only some fraction of a degree warmer during the growing season.
Additionally, higher latitudes may warm faster than lower latitudes \citep[e.g.][]{Hansen1997} so that a global mean temperature rise of 1 $^{\circ}$C would result in some growing seasons that are warmer than 1 $^{\circ}$C (in Canada, for example) and growing seasons that are only a fraction of a degree warmer in the tropics.
...
See \cite{Franke2020} for more details on GGCMI emulator construction for impact assessment.
...
The parametrization of growing seasons can have strong effects on simulated yields \citep{muller_global_2017, Jag2018} and it is essential for comparability of simulation results to harmonize between models.
...
The one exception to the harmonization protocol described above involves CARAIB, which kept their own growing season specifications rather than tuning to standard lengths.
...
As such, each model is run with seven different cultivar parameter sets in the A1 simulations (one per warming level) and the cultivar parameters in A0 and A1 are identical for T=0. 
...
For each scenario, 0.5 degree grid cell and crop, models provide 30-year timeseries of annual crop yields in units of tons ha$^{-1}$ year$^{-1}$, as well as total aboveground biomass yield; the dates of planting, anthesis, and maturity; applied irrigation water in irrigated scenarios; and total evapotranspiration. 
If models do not compute specific outputs, such as the anthesis date, these models can still participate in the GGCMI Phase II experiment. All models capable of computing the mandatory outputs are expected to also deliver these outputs..
...
\begin{table}[]
  \caption{Output variables, naming convention, and units in the GGCMI Phase II protocol. Items in \textbf{bold} are the mandatory minimum requirements (if model capacities allow for these outputs). Other variables are optionally provided depending on availability and participating modeling groups provided between 3 (PEPIC) and 18 (APSIM-UGOE) of these optional variables.}
  \label{table:outputs}
  \begin{tabular}{lll}
    \tophline \vspace{1mm}
...
The models participating in GGCMI phase II are listed in Table \ref{table:models}. 
...
Most models utilize either the Harmonized World Soil Database (HWSD) from the FAO \citep{FAOSOIL2008} or the ISRIC-WISE database \citep{BatjesL2008} or a derivation thereof.
...
For example, global correlation coefficients for maize in Phase I and Phase II are 0.89 and 0.74 for maize, 0.67 and 0.64 for wheat, and 0.64 and 0.59 for soybeans, respectively (Phase I values are from Figures 1--4 and 6 in \citet{muller_global_2017}).  
Differences in fidelity between regions and crops exceed differences between models: 
that is, Figure \ref{fig:simulation_val}(c)--\ref{fig:simulation_val}(f) shows more color similarity in horizontal than vertical bars. 
...
Yield reductions are due in part to shortenting of the growing season under warming and in part to the direct effects of higher temperature.
Models show some increases in high mountain regions that are currently cold-limited.
...
XXX - Something about irrigation water demand and the A1 dimension results???

See Supplemental Figures S8-10 for example reponses in the nitrogen and A1 dimensions and for a irrigation water demand example.
...
As such it also remains challenging to evaluate model performance as the actual growing conditions have to be interpolated between the uniform input levels used in the GGCMI Phase II setup. 
XXX-From Christoph, suggest move to Section 4.1 However, the models have mostly been evaluated in their global setup in the GGCMI Phase I evaluation exercise \citep{muller_global_2017} or have later used that online tool for the evaluation of subsequent model versions \citep[e.g.][]{von_Bloh_implementing_2018} and all models have been evaluated in the course of their development, including evaluations against site-specific experimental data.
GGCMI Phase II simulation results can be used for impacts projection, but only with the construction of an emulator of crop yield response to climatological changes, that is then driven by a realistic climate projection.
Note that while the experiment does not sample over potential changes in the higher-order moments in the distributions in temperature and precipitation, we shown these higher-order changes matter little for an impact assessment under a transient climate run such as an RCP from CMIP-5 when conducted with emulators developed from the GGCMI Phase II simulations \citep{Franke2020}.
