...
Those emulation studies all utilized temporal data from climate RCP runs and attempt to untangle correlations between the sensitivities to input drivers to crop models. The differences in year-to-year memory in the process-based crop models and the complexity of the changes in distributions in yearly weather under RCP scenarios in climate models are two complications we seek to control for with this study. Additional recent efforts have been made to generate datasets that allow more systematic sampling of the input variable space (also the focus of this study): \citet{Markowski2015} for temperature, CO$_2$, and nitrogen, \citet{Pirttioja2015} and \citet{Snyder2018} for temperature, water, and CO$_2$, and \citep{FRONZEK20182} for temperature and water, with all studies simulating selected sites for a limited number of crops. 

The use of limited input parameter space or restricted geographic scope may impede the ability to build future projections and to understand interaction effects in global process-based crop models. 

This is a conservative approach driven by the relative uncertainties surrounding the future climate. First moment projections in temperature are fundamentally made an energy balance standpoint, while changes in variability are made from a dynamics standpoint.
