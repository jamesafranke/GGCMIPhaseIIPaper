...
The overall goal of this study is a better understanding of global crop model response to the major drivers in a climate change context. Most previous climate-change-focused global crop modeling studies have simulated model response to representative concentration pathways (RCPs). RCPs are likely to have fairly strong covariance between precipitation, temperature and CO$_2$ that may be hard to decompose. We propose to test the response to major drivers and their interaction by isolating individual input drivers through simulations on first-moment shifts applied to the historical climatology instead of RCP simulations. As emulators are a 
