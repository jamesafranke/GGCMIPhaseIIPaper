...
\label{table:models}
	\begin{tabular}{p{6cm} p{1cm} p{1cm} p{1cm} p{1cm} p{1cm} p{1cm} p{1.5cm}}
        ...
        {Totals} & {12} & {10} & {11} & {9} & {12} & {10} & {5240 (e.g.\ maize)}\\
        \bottomhline
    \end{tabular}
\end{table*}
...
Not all modeling teams can provide the full simulation protocol, for instance, CARIAB and JULES do not simulate the nitrogen dimension and some crops are not parameterized in each model (see Table \ref{table:models} for details). Note that the three models that provide less than 50 simulations are excluded from the emulator analysis (APSIM-UGOE, EPIC-IIASA, and ORCHIDEE-crop). 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\codedataavailability{The resulting parameter matrices for all crop model emulators are available on {\color{red} give location?}, as are the raw simulation yield outputs. {\color{red} Code to run the emulator is available upon request to the author.}}
