...
Statistical emulation of crop simulations has been used to combine advantageous features of both statistical and process-based models...

In the past decade, many studies have developed emulators of crop yields from process-based models. Early studies proposing or describing potential emulators include A, B, C, D, and E. Studies developing single-model emulators include A for the XX model (for multiple crops in selected locations, using multiple scenarios as a training set). In recent years, emulators have begun to be used in the context of multi-model intercomparisons, with Blanc1, Blanc2, Ostberg, and Mistry using them to analyze the 5 crop models of the Inter-Sectoral Impacts Model Intercomparison Project (ISIMIP) (for maize, soy, wheat, and rice). Approaches differ: A and B used local weather variables (and CO2 values) and yields but emulate across broad soil types using a single climate scenario; Ostberg et al used only global mean temperature as a regressor but pattern-scale to emulate local yields using multiple climate scenarios; Mistry use local temperatures and yields and a historical simulation and compare with data; and Moore ..... The first four emulate year-over-year yield variations, while Moore develops a climatological damage function. In 2018, emulators were used for the first time to analyze non-realistic simulations that sampled a suite of climate perturbations: Snyder et al for temperature, water, and CO2, and Fronzek et al for temperature and water, both simulating selected sites for a limited number of crops.

The Global Gridded Crop Model Intercomparison (GGCMI) Phase II experiment seeks to provide a comprehensive dataset to allow systematically exploring how crop models respond to major climate and management drivers and their interactions. The experiment involves running a suite of process-based crop models across historical conditions perturbed by a set of defined input parameters, and was conducted as part of the Agr... (AgMIP), an international effort conducted under a framework similar to the Climate Model Intercomparison Project (CMIP) (  ). The GGCMI protocol builds on the AgMIP Coordinated Climate-Crop Modeling Project (C3MP) (Ruane ...) and will contribute to ....

[JOIN?]

GGCMI Phase II is designed to allow addressing goals such as understanding where   ; exploring future  ; understanding how interacting parameters ; quantifying uncertainties ; and testing strategies for producing lightweight emulators of process-based models. In this paper, we describe the GGCMI Phase II experiments, summarize output and present initial results, demonstrate that it is tractable to emulation, and present a simple climatological emulator as a potential tool for impacts assessments.
