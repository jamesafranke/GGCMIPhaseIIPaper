...
Additionally, even when provided with an appropriate local nitrogen level, models simulated \textit{potential} yields that do not included reductions from pests, weeds, or diseases. Potential yields represent an ideal case that is not realized in many less industrialized areas.
...
\citet{BLANC2015, BLANC2017} have shown that a fractional polynomial specification is more applicable for representing simulations at the yearly level across different soil types geographically. We do not test this specification here, and instead use a polynomial as it is the simplest effective specification to represent the climatological-mean response at the grid cell level.
