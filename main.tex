...
However, a common feature of all these efforts is that all use non-stationary simulations, in which input climate parameters are changing over time, and emulate annual crop yields across the entire future scenario or scenarios.  
...
For example, year-over-year yield variations may be dominated by changes in the distribution of temperatures throughout the growing season, while climate changes may be dominated by shifts in means. Regressing on the mean growing season temperature would produce different yield responses at the two timescales.  
...
early efforts in 2018 \citep{Markowski2015,Pirttioja2015}, and several recent studies in 2018 \citep{FRONZEK20182, Snyder2018}. Both \citet{FRONZEK20182} and \citet{Snyder2018} sample multiple perturbations to % XX check this !!
temperature and precipitation  (with \citet{Snyder2018} adding CO$_2$ as well), for 132 and 99 different combinations, respectively, and   
...
In this paper we describe a new comprehensive dataset designed to allow expand the parameter sweep approach still further. The Global Gridded Mode Intercomparison (GGCMI) Phase II experiment involves running a suite of process-based crop models across historical conditions perturbed by a set of defined input parameters. It provides global coverage at a half degree spatial resolution, and adds an applied nitrogen dimension to the parameter sampling, for a total of over 700 different combinations of input parameters for each model and crop. The experiment was conducted as part of the Agricultural Model Intercomparison and Improvement Project (AgMIP) \citep{ROSENZWEIG2013, Rosenzweig2014}, an international effort conducted under a framework similar to the Climate Model Intercomparison Project (CMIP) \citep{Taylor2012, Eyring2016}. The GGCMI protocol builds on the AgMIP Coordinated Climate-Crop Modeling Project (C3MP) \citep{ruane2014, mcdermid2015} and will contribute to the AgMIP Coordinated Global and Regional Assessments (CGRA) \citep{ruane2018, rosenzweig2018}. 
